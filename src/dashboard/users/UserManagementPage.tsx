...
const UserManagementPage: React.FC<UserManagementPageProps> = ({
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (isOnline()) {
        ...
        // Merge in local users not yet synced
        try {
          const localUsers = await getAllLocal("users") as User[] || [];
          for (const local of localUsers) {
            const match = users.find(u =>
              u.username === local.username &&
              u.firstName === local.firstName &&
              u.lastName === local.lastName
            );
            if (!match) users.push(local);
          }
        } catch {}
      } else {
        // Offline: load from IndexedDB only
        users = await getAllLocal("users") as User[] || [];
      }
  ...
  const handleAddUser = async () => {
    try {
      ...
      if (isOnline()) {
        ...
        await setLocal("users", offlineUser);
        await queueSync("add", "users", offlineUser);
      }
      setSnackbar({ 
        open: true, 
  ...
  // --- Accept user: support offline ---
  const handleAcceptUser = async (userId: string) => {
    try {
      if (isOnline()) {
        await acceptUser(userId);
      } else {
        // Offline: update local user and queue for sync
        ...
        if (localUser) {
          const updated = { ...localUser, status: "active" };
          await setLocal("users", updated);
          await queueSync("update", "users", updated);
        }
      }
      setSnackbar({ 
        open: true, 
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      if (isOnline()) {
        ...
        // Offline: update local user and queue for sync
        await setLocal("users", editUser);
        await queueSync("update", "users", editUser);
      }
      setSnackbar({ 
        open: true, 
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await deleteLocal("users", userToDelete.id);
        await queueSync("delete", "users", { id: userToDelete.id });
      }
      setSnackbar({ 
        open: true, 

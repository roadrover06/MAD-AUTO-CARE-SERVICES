...
const EmployeeManagementPage: React.FC<EmployeeManagementPageProps> = ({
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (isOnline()) {
        ...
        const snapshot = await getDocs(collection(db, "employees"));
        emps = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as Employee[];
        // Merge in local employees not yet synced
        try {
          const localEmps = await getAllLocal("employees") as Employee[] || [];
          for (const local of localEmps) {
            const match = emps.find(e =>
              e.firstName === local.firstName &&
              e.lastName === local.lastName
            );
            if (!match) emps.push(local);
          }
        } catch {}
      } else {
        // Offline: load from IndexedDB only
        emps = await getAllLocal("employees") as Employee[] || [];
      }
      setEmployees(emps);
    } catch (error) {
  ...
  const handleAddEmployee = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await setLocal("employees", { ...newEmployee, id: newId });
        await queueSync("add", "employees", { ...newEmployee, id: newId });
      }
      setSnackbar({
        open: true,
  ...
  const handleUpdateEmployee = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await setLocal("employees", selectedEmployee);
        await queueSync("update", "employees", selectedEmployee);
      }
      setSnackbar({
        open: true,
  ...
  // --- Delete employee: support offline ---
  const confirmDeleteEmployee = async () => {
    ...
    try {
      if (isOnline()) {
        ...
        await deleteLocal("employees", selectedEmployee.id);
        await queueSync("delete", "employees", { id: selectedEmployee.id });
      }
      setSnackbar({
        open: true,
  ...
  // Opens the delete dialog for the selected employee
